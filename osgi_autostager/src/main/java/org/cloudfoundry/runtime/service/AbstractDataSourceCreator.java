...
extends AbstractServiceCreator<DataSource, SI> {
	...
	@Override
	public BeanDefinition createBeanDefinition(SI serviceInfo) {
		// TODO Auto-generated method stub
		RootBeanDefinition beanDef = null;
		Class<?> beanClass = null;
		try {
			if (hasClass("org.apache.commons.dbcp.BasicDataSource"))
				beanClass = Class
						.forName("org.apache.commons.dbcp.BasicDataSource");
			else if (hasClass("org.apache.tomcat.dbcp.dbcp.BasicDataSource"))
				beanClass = Class
						.forName("org.apache.tomcat.dbcp.dbcp.BasicDataSource");
			else
				return null;
			beanDef = new RootBeanDefinition();
			beanDef.setBeanClass(beanClass);
			MutablePropertyValues pValues = new MutablePropertyValues();
			pValues.add("driverClassName", getDriverClassName());
			pValues.add("url", serviceInfo.getUrl());
			pValues.add("username", serviceInfo.getUserName());
			pValues.add("password", serviceInfo.getPassword());
			if (getValidationQuery() != null) {
				pValues.add("validationQuery", getValidationQuery());
				pValues.add("testOnBorrow", true);
			}
			beanDef.setPropertyValues(pValues);
