...
import org.cloudfoundry.reconfiguration.data.orm.JpaVendorConfigurer;
...
public class DataSourceConfigurer extends AbstractServiceConfigurer<RdbmsServiceInfo> {
	...
	private Configurer jpaVendorConfigurer;
	...
	//	this.jpaConfigurer = new JpaConfigurer(cloudEnvironment);
		this.jpaVendorConfigurer =new JpaVendorConfigurer(cloudEnvironment);
	...
	public boolean configure(DefaultListableBeanFactory defaultListableBeanFactory) {
		...
		if (configured) {
			jpaVendorConfigurer.configure(defaultListableBeanFactory);
