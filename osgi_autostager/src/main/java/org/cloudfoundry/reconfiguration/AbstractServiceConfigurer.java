...
abstract public class AbstractServiceConfigurer<SI extends AbstractServiceInfo>
	...
	public boolean configure(DefaultListableBeanFactory beanFactory) {
		...
		//			getServiceCreator().createSingletonService(cloudServices.get(0)).service);
			//beanFactory.registerBeanDefinition(getServiceBeanName(), getServiceCreator().createBeanDefinition(cloudServices.get(0)));
			BeanDefinition newBean = getServiceCreator().createBeanDefinition(cloudServices.get(0));
			BeanDefinition bdef =  beanFactory.getBeanDefinition(beanNames[0]);
			
			for(PropertyValue pv:newBean.getPropertyValues().getPropertyValues()){
				((AbstractBeanDefinition)bdef).getPropertyValues().removePropertyValue(pv.getName());
				((AbstractBeanDefinition)bdef).getPropertyValues().addPropertyValue(pv.getName(), pv.getValue());				
			}
			return true;
